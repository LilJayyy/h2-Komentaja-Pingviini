...
Alla olevalla komennolla lisäsin suoraan terminalin kautta testitiedostoon tekstisisällön.

echo -komennolla:
* `echo "testailen 2" > testitiedosto.txt`
 ![teksti](images/teksti.png)

Nano-editorin kautta:
* `nano testitiedosto.txt`
* tallennus tapahtuu **ctrl + O** -näppäimillä. Enterillä siirrytään taas seuraavalle riville ja **ctrl + X** poistutaan nano-editorista.

 ![nanot](images/nanot.png)
...
Syötämme putkeen A syötteen, joka siirtyy (|) avulla putkeen B.

Alla oma esimerkkini, joka avasi minulle hyvin asiaa.

* `ls | grep testi`
* etsimme siis grepillä putken avulla kaikki testi -sanan sisältävät paikat.
...
Lähdin listaamaan testaamani koneen rautaa. Tässä meni muutama minuutti, kunnes havaitsin kirjoitusvirheen. `apt install lshw` kun piti olla `sudo apt install lshw` -komento.  

Vihdoin, alkoikin asennus salasanan syötön jälkeen.
...
*`journalctl -f` -komennolla kaikki lokitiedot alkuun esille

* Journalctl --since "2025-08-28 19:00:00" -komennolla
*Lokitapahtumat listataan syötetyn päivämäärän alkmispäivästä alkaen*
*Istunto käynnistyi 19:13 alkaen kuvan mukaisesti*

 ![since](images/since.png)

**Itse tulkintaan** 

*`journalctl --list-boots` -komento ja lokirivien tulkinta 
Tällä oli helpoin aloittaa. Listalla nähdään seuraavia asioita vasemmalta oikealle:
...
*`journalctl _UID=1000` -komento ja lokirivien tulkinta
Tässä auttoi hieman se, että suurin osa lokitiedoista oli vain ns. "perustietoja" jotka oli helppo tulkita. Muita joutui hieman ohjeista etsimään ja Googlettamalla, lähteet raportin lopussa.

Vasemmalta ensimmäisenä:
...
Huomasin Githubissa hyvän ohjeen (Karvinen, T.) Micro-editorin pluginin Paletteron asentamiseen. 
